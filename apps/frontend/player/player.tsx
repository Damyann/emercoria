...
export default function PlayerPage(){
  ...
  useEffect(()=>{const next=readSession();if(!next){router.replace('/login');return;}if(next.user.accountType!=='PLAYER'){router.replace('/admin');return;}setReady(true);},[router]);
  useEffect(()=>()=>{if(avatarUrl) URL.revokeObjectURL(avatarUrl);},[avatarUrl]);

  if(!ready) return <main className='min-h-screen bg-[#02080d]' />;

  const leftPanel={width:250,minWidth:250,flex:'0 0 250px',borderRight:'1px solid #000',overflow:'visible'} as const;
  ...
  const mainPanelClosedHeight=135;
  const mainPanelOpenHeight=550;

  const pickAvatar=()=>inputRef.current?.click();
  const setAvatar=(e:React.ChangeEvent<HTMLInputElement>)=>{const file=e.target.files?.[0];if(!file||!file.type.startsWith('image/')) return;setAvatarUrl(prev=>{if(prev) URL.revokeObjectURL(prev);return URL.createObjectURL(file);});e.target.value='';};
  ...
  return(
    <main className='min-h-screen text-[#1b1d21]' style={{backgroundImage:`url(${background.src})`,backgroundSize:'cover',backgroundPosition:'center',backgroundRepeat:'no-repeat',backgroundAttachment:'fixed'}}>
      <div className='mx-auto flex min-h-screen flex-col' style={{width:'70%'}}>
        ...
        {/* Header */}
        <header className='flex h-[70px] shrink-0 items-center border-x border-b border-black/10 bg-[#f7f7f8] pr-4'>
        ...
        <section className='flex-1 border-l border-b border-black bg-[#f7f7f8]' style={{display:'flex',minHeight:'calc(100vh - 70px)'}}>
          ...
          <aside aria-label='left panel' style={leftPanel}>
            <div className='flex w-full flex-col items-center pt-0'>
              ...
              {/* Dropdown main panel */}
              <div style={{position:'relative',width:250,height:mainPanelOpen?mainPanelOpenHeight:mainPanelClosedHeight,flex:`0 0 ${mainPanelOpen?mainPanelOpenHeight:mainPanelClosedHeight}px`,overflow:'hidden',marginTop:-4,transition:'height .25s ease, flex-basis .25s ease'}}>
                <Image src={leftPanelMain} alt='Player main panel' width={250} height={550} className='block h-[550px] w-[250px] object-fill' sizes='250px' style={{position:'absolute',left:0,top:0,zIndex:1}}/>
                <button type='button' aria-label={mainPanelOpen?'Затвори менюто':'Отвори менюто'} aria-expanded={mainPanelOpen} onClick={()=>setMainPanelOpen(prev=>!prev)} style={{position:'absolute',left:0,top:0,width:250,height:mainPanelClosedHeight,zIndex:3,border:0,outline:'none',background:'transparent',cursor:'pointer',padding:0}}/>
              </div>
