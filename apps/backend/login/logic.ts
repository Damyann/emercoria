...
function signToken(payload:Record<string,unknown>){
  ...
  const body=encode({ iss:process.env.JWT_ISSUER||'backend', iat:now, exp:now+ttl, ...payload });
  const signature=createHmac('sha256', secret).update(`${header}.${body}`).digest('base64url');
  return { accessToken:`${header}.${body}.${signature}`, expiresIn:ttl };
}
...
export class LoginLogic implements OnModuleDestroy {
  ...
  async login({ email, password }: LoginDto) {
    ...
    return {
      ...token,
      tokenType:'Bearer',
      user:{ id:user.id, email:user.email, nickname:user.nickname, accountType:user.accountType, moderatorLevel:user.moderatorLevel },
    };
  }
}
